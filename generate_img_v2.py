...
for index in range(10000):
    ...
    label = np.zeros([130, 5])
    l = len(sketch)
    label[0:l, 0:2] = sketch[:, 0:2]
    label[0:l, 3] = sketch[:, 2]
    label[0:l, 2] = 1 - label[0:l, 3]
    label[l:, 4] = 1

    idx = np.random.randint(1, 130)
    ...
    cv2.imencode('.jpg', img2)[1].tofile(p.train_path + str(index) + ".jpg")

    print(index)

print(np.shape(label_list))
np.save(p.label_path + "label", np.asarray(label_list))
