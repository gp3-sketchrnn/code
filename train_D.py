...
if __name__ == '__main__':
    t_x, t_y = get_data()
    train_ds = (_input_fn(t_x[:900], tf.nn.softmax(t_y[:900]), 900))
    validation_ds = _input_fn(t_x[900:], tf.nn.softmax(t_y[900:]), 100)
    ...
    prediction_layer = tf.keras.layers.Dense(500, activation=tf.nn.softmax)
