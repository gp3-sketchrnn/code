...
def sample(sess, model, cnn_model,  seq_len=250, temperature=1.0, greedy_mode=False, z=None):
    ...
    strokes_1 = np.zeros((seq_len, 5), dtype=np.float32)
    mixture_params = []
    ...
    for i in range(seq_len):
        ...
        # if i % 3 == 2:
        #     img = sketch_utils.sketch_2_img(strokes[:i])
        #     nt_x1, nt_x2 = sketch_utils.get_point(cnn_model, img)
        #     strokes_1[i, :] = [nt_x1, nt_x2, eos[0], eos[1], eos[2]]
        if i > 5:
            img = sketch_utils.sketch_2_img(strokes[:i])
            c_pi, c_mu1, c_mu2, c_sigma1, c_sigma2, c_corr, c_pen = sketch_utils.get_suggested_point(cnn_model, img)
            k = 0.5
            n_pi = c_pi * k + o_pi * (1 - k)
            n_mu1 = c_mu1 * k + o_mu1 * (1 - k)
            n_mu2 = c_mu2 * k + o_mu2 * (1 - k)
            n_sigma1 = c_sigma1 * k + o_sigma1 * (1 - k)
            n_sigma2 = c_sigma2 * k + o_sigma2 * (1 - k)
            n_corr = c_corr * k + o_corr * (1 - k)
            n_pen = o_pen

            n_idx = get_pi_idx(random.random(), n_pi[0], temp, greedy)

            n_idx_eos = get_pi_idx(random.random(), n_pen[0], temp, greedy)
            n_eos = [0, 0, 0]
            n_eos[n_idx_eos] = 1

            nt_x1, nt_x2 = sample_gaussian_2d(n_mu1[0][idx], n_mu2[0][idx], n_sigma1[0][idx], n_sigma2[0][idx],
                                                  n_corr[0][idx], np.sqrt(temp), greedy)
            strokes_1[i, :] = [nt_x1, nt_x2, n_eos[0], n_eos[1], n_eos[2]]
        else:
            strokes_1[i, :] = [next_x1, next_x2, eos[0], eos[1], eos[2]]
    ...
    return strokes, strokes_1, mixture_params
